		...
		<div class="nav-collapse collapse">
			...
			<ul class="nav pull-left">
				<?php 
					...
					$subNav = array (
						"Node" => array (
							"List Nodes" => site_url("node"),
						),
						"Content Type" => array (
							"Add Content Type" => site_url("add/content_type"),
						)
					);
					foreach($contentTypes->result() as $contentType){
						$subNav["Node"]["Add ".$contentType->name] = site_url("add/node/".$contentType->id);
						$subNav["Content Type"]["Edit ".$contentType->name] = site_url("edit/content_type/".$contentType->id);
					}
					foreach ($navigation as $name => $url){
						...
						if ($this->Permission->level($minPermission)){
							...
							if (($this->uri->segment(1) == $urlSegments[0])&&($this->uri->segment(2) == $urlSegments[1])){
								...
								<a href="<?php echo site_url($url); ?>"<?php
								...
								<?php } else { ?>
									...
									<ul class="dropdown-menu">
										<?php foreach ($subNav[$name] as $subNavItem => $subNavLink) { ?>
											<li><a href="<?php echo $subNavLink ?>"><?php echo $subNavItem ?></a></li>
										<?php } ?>
									</ul>
								<?php } ?>
							</li>
